...
# Exclude sites that haven't submitted in the past 30 days
EXCLUDED_SITES_ARR=( $(cut -d ',' -f1 ./spec/_data/excl_sites.csv) )
...
do
  ...
  if [[ "${EXCLUDED_SITES_ARR[@]}" =~ "${i}" ]]; then
    echo "  schedule: ${YESTERDAY_SCHEDULE_LOWER}" >> ${CRON_YAML}
  else
    echo "  schedule: $(date -v+${time_diff}M '+%e of %B %H:%M' | tr '[:upper:]' '[:lower:]')" >> ${CRON_YAML}
    ((time_diff=time_diff+20))
  fi
  echo "  timezone: America/New_York" >> ${CRON_YAML}
done
